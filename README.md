...
![LOGO](https://github.com/SimBongSa/Back-end/blob/main/Vongole/src/main/resources/static/%EB%A1%9C%EA%B3%A0600-340.png)

##  1. 프로젝트 한 줄 설명
빠른 봉사인원 모집 및 참여 가능한 캘린더⭐

[발표 영상 링크](https://youtu.be/L8mj6ucXfFw)
...
1. board 테이블 문자열 컬럼에 저장
2. board 테이블 list 컬럼에 저장
3. hashtag 테이블 새로 생성해서 board pk값과 함께 저장
...
### 🎯 for-loop -> stream
중간 발표 이후로 for 문을 stream으로 바꿔 보라는 피드백을 받았다. 항해를 시작하기 이전부터 stream이 성능이 좋지 않다는 괴소문(?)을 들어왔던 터라 일부러 stream을 사용하지 않아왔었는데 이번 기회에 for-loop와 stream 성능 비교를 해보았다.
